...
class ProjectBase:
    ...
    def insert_country(self):
        ...
        country_data = pd.read_json("../../data/country.json")
    ...
    def insert_project_surveyor(self):
        ...
        surveyor_data["village_id"] = surveyor_data["village"].apply(
            lambda x: engine.execute(
                "SELECT id FROM village WHERE name = '{}'".format(x)
            ).fetchone()[0]
        )
    ...
    def create_project(self):
        ...
        project_data = pd.read_json("../../data/project_data.json")
...
cook_stove_data = clean_project_file.clean_project_file(file_path="../../app/data/co2balance.xlsx", sheet_names=["Day 1 Cleaned"])
