...
def extract_data(**kwargs):
    fn = extract_project_data(
        file_path='/opt/airflow/dags/app/data/co2balance.xlsx', sheet_names=['Day 1 Cleaned', 'Day 2 Cleaned'])
    kwargs['ti'].xcom_push(key='extracted_data_file', value=fn)


def transform_data(**kwargs):
    ti = kwargs['ti']
    extracted_data_file = ti.xcom_pull(
        key='extracted_data_file', task_ids='ExtractData')
...
def load_project_base(**kwargs):
    ti = kwargs['ti']
    transformed_data_file = ti.xcom_pull(
        key='transformed_data_file', task_ids='TransformData')
